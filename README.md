...
描述了美国纽约州（New York State）全境在普查区 (Census Tract) 粒度下的人口分布、地理空间位置及区域间的人群流动网络。数据经过预处理和清洗，采用 Python 原生序列化格式 (.pkl) 存储，旨在支持空间分析、城市计算、交通预测及流行病传播模拟等任务。
...
理解 FIPS 代码 (Federal Information Processing Standards)。所有的 Key 均遵循 11 位数字字符串格式 SSCCCTTTTTT：
...
节点一致性：centroid.pkl 和 population.pkl 的 Key 集合应当是高度重合的（通常是一一对应），构成了图的节点集合 $V$。

边的一致性：od2flow.pkl 中的所有 Origin 和 Destination ID 都必须存在于 centroid.pkl 的 Key 集合中（除了极少数可能因版本差异导致的孤立点）。
...
测试数据集 (Markdown / Python Code)

## 1. Centroid Data (中心点坐标)

数据含义：普查区ID映射到 [经度, 纬度]（注意：根据源数据，这里是经度在前，纬度在后。原始数据中[-77.xxx, 43.xxx]，即 [Lon, Lat]）。

```python
# test_centroid_data
centroid = {
    '36061000100': [-74.0461708, 40.6900008],       # 曼哈顿 (自由女神像附近区域)
    '36055011703': [-77.4008057, 43.1028109],       # 门罗县 (Monroe County)
    '36069050101': [-77.4582906, 43.0059497],       # 安大略县 (Ontario County)
    '36055014003': [-77.68674509999998, 43.216090200000004], # 门罗县 (Monroe County)
    '36075021602': [-76.5295124, 43.4510055]        # 奥斯威戈县 (Oswego County)
}
```
## 2. Population Data (人口数据)
...
population = {
    '36061000100': 0,     # 某些商业区或特定区域（如公园/地标）可能常住人口极少或为0
    '36055011703': 4251,
    '36069050101': 3298,
    '36055014003': 4641,
    '36075021602': 2821
...
```
## 3. OD to Flow Data (OD流量数据)

数据含义：(起点ID, 终点ID) 元组映射到流量数值。

只保留了起点和终点都在上述 5 个节点内的流量数据，形成了一个闭环网络。
...
| 片段 | 长度 | 含义 | 示例值 | 说明 |
| --- | --- | --- | --- | --- |
| SS | 2位 | 州代码 (State) | 36 | 代表纽约州 (New York)。本数据集中所有 Key 均以 36 开头。 |
| CCC | 3位 | 县代码 (County) | 061 | 代表具体的县。见下方对照表。 |
| TTTTTT | 6位 | 普查区 (Tract) | 000100 | 县内部的具体普查区编号。 |
